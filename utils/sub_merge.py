...
class SubMerge:
    ...
    def geoip_update(url: str) -> None:
        """更新 GeoIP 数据库"""
        import os
        if os.path.exists("./utils/Country.mmdb"):
            print("Country.mmdb already exists, skipping download.\n")
            return
    ...
    def main(url_list: list[dict[str, Any]]) -> None:
        ...
        for index, item in enumerate(url_list):
            ...
            if content == "Url 解析错误":
                ...
                if content != "Url 解析错误":
                    ...
                    print(f"Writing content of {remark} to {index:0>2d}.txt\n")
                else:
                    print(f"Writing error of {remark} to {index:0>2d}.txt because of Url 解析错误\n")
            elif content == "Url 订阅内容无法解析":
                print(f"Writing error of {remark} to {index:0>2d}.txt because of Url 订阅内容无法解析\n")
        ...
        content_yaml = SubConvert.main(content_raw, "content", "YAML", {"dup_rm_enabled": False, "format_name_enabled": True})
        content_base64 = SubConvert.base64_encode(content_raw)

        SubMerge._write_content("./sub", content_base64)  # 根目录订阅文件写入
        SubMerge._write_content("./sub.yaml", content_yaml)
        print("Done!\n")
